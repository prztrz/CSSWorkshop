...
gulp.task ('sass', function() {
    return gulp.src('scss/**/*.scss') // wskazujemy skąd brać pliki scss - /**/ ze wszystkich podfolderach *.scss - wszystkie *.scss
    .pipe(sourceMaps.init()) //zaczyna działanie sourcemaps
    .pipe(sass({
        outputStyle: 'expanded'
    })) //wywołanie funkcji sass (zdefiniowanej w linijce 2) która kompiluje jw; pipe określa kolenośc
...
gulp.task ('default', function() {
    gulp.start('sass');
    gulp.watch('scss/*.scss', ['sass']) // przy każdym zapisaniu pliku *.scss uruchamia zadanie sass
